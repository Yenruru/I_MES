...
define(["angularAMD", "moment"], function (angularAMD, moment) {
	...
	angularAMD.service("WebService", function($http, $state, $rootScope, config, $soap, $translate){
		...
		//----------------------------------------------------------------------------------------------
		//Function Call Pro 6 Invoke
		function Invoke(httpData, showLoading) {
			//2017.06.21 yenru , 新增語系傳入
			//-------------------------------
			var strlanguage;
			if (config.setting.lang == 'zh_CN')
            ...
            //2018.01.30, Yenru 傳入Method為連線測試 param 不傳入 XML 改傳UserNo
            if (InvokeMethod == "Test")
            {param = {UserNo:"IMES"};}
            else 
            {param = {method:httpData.method,inXMl:json2xml(param)};} 
			...
			if(showLoading)
				$rootScope.showLoading();
			try{
				$soap.post("http://"+config.server.ip + "/" + hostName + "/wsInvoke.asmx" ,InvokeMethod , param, function(){
					if(showLoading)
						$rootScope.hideLoading();
					$rootScope.showAlert("無法與伺服器連線",1);
				}).then(function(response){
					//console.log(response);
					if(showLoading)
						$rootScope.hideLoading();
...
　						var NewArray = data.split(",");
		...
		this.Invoke = function(httpData, showLoading){
			hostName = config.setting.hostName;
            InvokeMethod = "invokeSrv_Xml"
			Invoke(httpData, showLoading);
		}
		
		this.Invoke_Session = function(httpData, showLoading){
			hostName = config.setting.hostName;
            InvokeMethod = "invokeSrv_Xml_Session"
			Invoke(httpData, showLoading);
		}

        this.Test = function(httpData, showLoading){
			hostName = config.setting.hostName;
            InvokeMethod = "Test"
			Invoke(httpData, showLoading);
		}
